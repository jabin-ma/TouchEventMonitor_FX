...
public final class AndroidDebugBridge {
    ...
    private static final ArrayList<IDebugBridgeChangeListener> sBridgeListeners = new ArrayList<IDebugBridgeChangeListener>();
    private static final ArrayList<IDeviceChangeListener> sDeviceListeners = new ArrayList<IDeviceChangeListener>();
    private static final ArrayList<IClientChangeListener> sClientListeners = new ArrayList<IClientChangeListener>();
    ...
    public interface IClientChangeListener {
         ...
         * @param changeMask the bit mask describing the changed properties. It can
    ...
    public static AndroidDebugBridge createBridge() {
        synchronized (sLock) {
            ...
            IDebugBridgeChangeListener[] listenersCopy = sBridgeListeners
                    .toArray(new IDebugBridgeChangeListener[sBridgeListeners.size()]);
     ...
     * @param forceNewBridge force creation of a new bridge even if one with the same
    ...
    /**
     * Sets the client to accept debugger connection on the custom "Selected
     * debug port".
     *
    ...
    /**
     * Queries adb for its version number and checks that it is atleast
     * {@link #MIN_ADB_VERSION}.
     */
    private void checkAdbVersion() throws IOException {
        ...
        if (version.compareTo(MIN_ADB_VERSION) > 0) {
            ...
            String message = String.format("Required minimum version of adb: %1$s." + "Current version is %2$s",
    ...
    synchronized boolean startAdb() {
        ...
        if (status != 0) {
            Log.e(DDMS, String.format("'%1$s' failed -- run manually if necessary", commandString)); //$NON-NLS-1$
            return false;
        } else {
     ...
     * <li>Defaults to {@link #DEFAULT_ADB_PORT} if neither the system property
     * nor the env var are set.</li>
     * </ol>
     *
    ...
    private static int getAdbServerPort() {
        ...
        if (prop != null) {
            try {
                ...
                String msg = String.format("Invalid value (%1$s) for ANDROID_ADB_SERVER_PORT system property.", prop);
                Log.w(DDMS, msg);
            }
        }

        // when system property is not set or is invalid, parse environment
        // property
        try {
            ...
            Log.w(DDMS, "No access to env variables allowed by current security manager. "
                    + "If you've set ANDROID_ADB_SERVER_PORT: it's being ignored.");
        } catch (IllegalArgumentException e) {
            String msg = String.format("Invalid value (%1$s) for ANDROID_ADB_SERVER_PORT environment variable (%2$s).",
                    prop, e.getMessage());
            Log.w(DDMS, msg);
        }
    ...
    public String toString() {
        return "========AndroidDebugBridge========\nmAdbOsLocation=" + mAdbOsLocation + "\nmVersionCheck="
                + mVersionCheck + "\nmStarted=" + mStarted + "\nmDeviceMonitor=" + mDeviceMonitor + "\nisConnected()="
                + isConnected() + "\ngetConnectionAttemptCount()=" + getConnectionAttemptCount()
                + "\ngetRestartAttemptCount()=" + getRestartAttemptCount();
    }
}
