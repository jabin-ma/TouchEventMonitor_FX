...
public final class Message implements Delayed {
	 ...
	 * valid for messages posted by a {@link Messenger}; otherwise, it will be
	 * -1.
	 */
	 ...
	 * and remains set while it is delivered and afterwards when it is recycled.
	 * The flag is only cleared when a new message is created or obtained since
	 * that is the only time that applications are allowed to modify the
	 * contents of the message.
	 *
	 * It is an error to attempt to enqueue or recycle a message that is already
	 * in use.
	 */
	 ...
	 * currently enqueued or that is in the process of being delivered to a
	 * Handler.
	 * </p>
	 */
	public void recycle() {
	...
	/**
	 * Recycles a Message that may be in-use. Used internally by the
	 * MessageQueue and Looper when disposing of queued Messages.
	 */
	...
	/**
	 * Sets whether the message is asynchronous, meaning that it is not subject
	 * to {@link Looper} synchronization barriers.
	 * <p>
	 * Certain operations, such as view invalidation, may introduce
	 * synchronization barriers into the {@link Looper}'s message queue to
	 * prevent subsequent messages from being delivered until some condition is
	 * met. In the case of view invalidation, messages which are posted after a
	 * call to {@link android.view.View#invalidate} are suspended by means of a
	 * synchronization barrier until the next frame is ready to be drawn. The
	 * synchronization barrier ensures that the invalidation request is
	 * completely handled before resuming.
	 * </p>
	 * <p>
	 * Asynchronous messages are exempt from synchronization barriers. They
	 * typically represent interrupts, input events, and other signals that must
	 * be handled independently even while other work has been suspended.
	 * </p>
	 * <p>
	 * Note that asynchronous messages may be delivered out of order with
	 * respect to synchronous messages although they are always delivered in
	 * order among themselves. If the relative order of these messages matters
	 * then they probably should not be asynchronous in the first place. Use
	 * with caution.
	 * </p>
	 *
	 * @param async
	...
	public Message() {
	 ...
	 void submit(long newdelay) {
		this.delay = newdelay;
		submitTime = System.currentTimeMillis()+ this.delay;
	}
